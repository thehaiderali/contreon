...
// PostSettings.jsx (Updated for both Create and Edit)
import React, { useState, useEffect } from 'react'
import { Label } from "@/components/ui/label"
import { Switch } from "@/components/ui/switch"
import { Button } from "@/components/ui/button"
...
export const PostSettings = ({ 
  ...
  useEffect(() => {
    if (!isInitialized && initialSettings) {
      setVisibility(initialSettings.isPaid ? 'paid' : 'public')
      setCommentsAllowed(initialSettings.commentsAllowed !== false)
      setSelectedTierId(initialSettings.tierId || "")
  ...
  return (
    <Card className="w-full border-2 border-black dark:border-white">
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-[9px] text-muted-foreground text-center">
          {isSubmitting 
            ? (isEditMode ? 'Your post is being updated...' : 'Your post is being created...') 
            : 'Review settings and click to continue'}
        </p>
      </CardContent>
    </Card>
  )
