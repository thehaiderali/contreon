                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex justify-between items-baseline">
                      <span className="font-medium text-sm truncate">{sub.fullName}</span>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="relative">
              <Avatar className="h-9 md:h-10 w-9 md:w-10">
                ...
                <AvatarFallback>{selectedSubscriber ? getInitials(selectedSubscriber.fullName) : '?'}</AvatarFallback>
              </Avatar>
              {selectedSubscriber && onlineStatus[getUserId(selectedSubscriber._id)] && (
            ...
            <div className="hidden md:block">
              ...
              <p className="text-xs text-muted-foreground">
                {selectedSubscriber ? (onlineStatus[getUserId(selectedSubscriber._id)] ? "Online" : "Offline") : ""}
              </p>
            </div>
            <div className="md:hidden">
              <p className="font-medium text-sm">{selectedSubscriber?.fullName}</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-2 max-w-3xl mx-auto">
            {loadingMessages ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                ...
                <p className="text-sm">Select a subscriber to start chatting</p>
              </div>
            ) : messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center mb-4">
              ...
              <>
                {messages.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg._id || idx} className={`flex ${isCreator ? "justify-end" : "justify-start"} gap-1 md:gap-2`}>
                      {!isCreator && showAvatar && (
                        <Avatar className="h-7 md:h-8 w-7 md:w-8 mt-1">
                          <AvatarFallback className="text-xs">
                            {selectedSubscriber ? getInitials(selectedSubscriber.fullName) : '?'}
                          </AvatarFallback>
                        </Avatar>
                      )}
                      <div className={`max-w-[75%] md:max-w-[70%] ${!isCreator && !showAvatar ? "ml-8 md:ml-10" : ""}`}>
                        <div
